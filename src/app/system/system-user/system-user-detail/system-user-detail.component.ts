  ...
  implements OnInit, OnChanges
{
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.detail && this.detailForm) {
      ...
      if (isObjectEmpty(changes.detail.currentValue)) {
        ...
        this.detailForm
          .get('userPassword')
          .setValidators([FormValidator.required, FormValidator.maxLength(12)]);
  ...
  async updateUserActiveYn(event: Event, userActiveYn: string): Promise<void> {
    const confirm = await this.messageService.confirm1(
      `계정을 ${userActiveYn === 'Y' ? '잠금해제하시겠어요?' : '잠그시겠어요?'}`
    );
    if (!confirm) return;
    ...
    this.userService
      .updateUser$({
        ...
        userActiveYn,
        actionType: 'UPDATE_USER_ACTIVE_YN',
      })
      .subscribe((response) => {
  ...
  async onRemove(event: Event): Promise<void> {
    const confirm = await this.messageService.confirm2(
      '사용자를 삭제하시겠어요?<br>이 작업은 복구할 수 없어요.'
    );
    if (!confirm) return;

    this.userService.removeUser$(this.detail.userId).subscribe(() => {
  ...
  async superLogin(): Promise<void> {
    const confirm = await this.messageService.confirm1(
      `${this.detail.userAccount}(${this.detail.employee.employeeName}님) 계정으로 로그인하시겠어요?`
    );
    if (!confirm) return;
  ...
  async resetPassword(): Promise<void> {
    if (isBlank(this.detail.employee.emailAddr)) {
      this.messageService.toastInfo(
        '이메일주소가 등록되어 있지 않아요.<br>이메일주소를 등록한 후 다시 시도해주세요.'
      );
      return;
    }

    const confirm = await this.messageService.confirm1(
      `임시 비밀번호를 ${this.detail.userAccount}(${this.detail.employee.employeeName}님) 메일로 발송하시겠어요?`
    );
    if (!confirm) return;
    ...
    this.authService
      ...
      .subscribe(() => {
        this.messageService.toastSuccess(
          `임시 비밀번호를 ${this.detail.userAccount}(${this.detail.employee.employeeName}님) 메일로 발송했어요.`
        );
      });
  }
